...
export async function createLink(request: Request, env: Env) {
  ...
  await env.DB.prepare('INSERT INTO links (id, group_id, title, url, icon, description, sort_order, pinned, archived, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
    .bind(row.id, row.group_id, row.title, row.url, row.icon, row.description, row.sort_order, row.pinned, row.archived, row.created_at, row.updated_at)
    .run()
...
export async function updateLink(request: Request, env: Env, id: string) {
  ...
  await env.DB.prepare('UPDATE links SET group_id = ?, title = ?, url = ?, icon = ?, description = ?, sort_order = ?, pinned = ?, archived = ?, updated_at = ? WHERE id = ?')
    .bind(updated.group_id, updated.title, updated.url, updated.icon, updated.description, updated.sort_order, updated.pinned, updated.archived, updated.updated_at, id)
    .run()
